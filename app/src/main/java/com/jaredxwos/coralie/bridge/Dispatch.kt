...
import com.jaredxwos.coralie.mesh.meshReset
...
suspend fun dispatch(message: WebMessageCompat): BridgeResponse {
    ...
    val request = try {
        ...
        return BridgeResponse(
            BridgeStatus.ERROR,
            null,
            JsonPrimitive("Malformed request: ${e.message}"),
        )
    }

    val handler = callbacks[request.callback]
        ?: return BridgeResponse(
            BridgeStatus.ERROR,
            request.id,
            JsonPrimitive("Callback not registered"),
        )
    ...
    val result = try {
        ...
        return BridgeResponse(
            BridgeStatus.ERROR,
            request.id,
            JsonPrimitive("Callback encountered an error. ${e.message}"),
        )
    }
